...
onready var jobsList = $JobElements/JobsMenu
...
func start_working():
	...
	selectedJob.text = selectedJobName % SELECTED_JOB.name
	workingProgress.max_value = SELECTED_JOB.time
	jobTimer.start(1)
...
func create_jobs():
	...
	for job in jobsInventory:
		jobsList.add_item(job.name,job.id)
